...
var Category = ((Category2) => (
        ...
        (Category2.FRAMEWORK_WEB_COMPONENTS_VITE =
            ...
            case 'array':
                return !type.value || !Array.isArray(arg)
                    ...
                    : arg.reduce((acc, item, index) => {
                          ...
                          return (
                              mapped !== INCOMPATIBLE && (acc[index] = mapped),
                              acc
                          );
...
function prepareStory(
    ...
    let { moduleExport, id, name: name2 } = storyAnnotations || {},
        ...
        undecoratedStoryFn = (context) => {
            let { passArgsFirst: renderTimePassArgsFirst = !0 } =
                context.parameters;
...
function prepareContext(context) {
    ...
    let mappedArgs = Object.entries(targetedContext.args).reduce(
        ...
        includedArgs = Object.entries(mappedArgs).reduce((acc, [key2, val]) => {
            ...
            return (
                v(argType, mappedArgs, targetedContext.globals) &&
                    (acc[key2] = val),
                acc
            );
        }, {});
    ...
    StoryStore = class {
        ...
        preparedMetaFromCSFFile({ csfFile }) {
            if (!this.projectAnnotations)
                throw new Error(
                    'storyFromCSFFile called before initialization',
                );
            ...
            let storyImports =
                ...
                [entryExports, ...csfFiles] = await Promise.all([
                    ...
                    ...storyImports.map((storyImportPath) => {
                        let firstStoryEntry =
                            storyIndex.importPathToEntry(storyImportPath);
        ...
        getSetIndexPayload() {
            ...
            return {
                ...
                entries: Object.fromEntries(
                    Object.entries(this.storyIndex.entries).map(
                        ([id, entry]) => [
                            id,
                            stories[id]
                                ? {
                                ...
                                : {
                                      ...
                                      parameters: this.preparedMetaFromCSFFile({
                                          csfFile:
                                              cachedCSFFiles[entry.importPath],
                                      }).parameters,
                                  },
                        ],
                    ),
...
function useSharedState(sharedId, defaultState) {
    let channel = addons.getChannel(),
        ...
        allListeners = useMemo(
            () => ({
                [`${SHARED_STATE_CHANGED}-manager-${sharedId}`]: (s) =>
                    setState(s),
...
var import_synchronous_promise2 = __toESM(require_synchronous_promise(), 1),
    ...
    StoryStoreFacade = class {
        ...
        legacy (6.x) MDX API, or use the new store.`);
            ...
            if (
                ((title = userOrAutoTitle(fileName, specifiers, title)), !title)
            ) {
                logger.info(
            ...
            let storyExports = Object.entries(sortedExports).filter(([key2]) =>
                ...
                autodocsOptedIn =
                    autodocs === !0 ||
                    (autodocs === 'tag' && componentAutodocs);
            if (
                storyExports.length &&
            ...
            storyExports.forEach(([key2, storyExport]) => {
                let exportName = M(key2),
                    id =
                        storyExport.parameters?.__id ||
                        N(componentId || title, exportName),
                    name2 =
        ...
        constructor({ storyStore } = {}) {
            (this.lastFileName = 0),
                ...
                (this.addArgsEnhancer = (enhancer) => {
                    this.facade.projectAnnotations.argsEnhancers?.push(
                        enhancer,
                    );
                }),
    ...
    Preview = class {
        ...
        emitGlobals() {
            ...
            let payload = {
                ...
                globalTypes:
                    this.storyStore.projectAnnotations.globalTypes || {},
            };
